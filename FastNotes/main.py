...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # set style sheet
        self.setStyleSheet('background-color:#4e4e4e;color:#f7f7f7;')
        ...
        self.TextOne = QPlainTextEdit()

        # setting font to TextOne
        fixedfont = QFontDatabase.systemFont(QFontDatabase.FixedFont)
        fixedfont.setPointSize(12)

        # set fixed font to each text TextOne
        self.TextOne.setFont(fixedfont)

        # self.path holds the path of the currently open file.
        # If none, we haven't got a file open yet (or creating new).
        self.path = None

        # adding TextOne and to the layout
        layout.addWidget(self.m_w11, 1, 0, 1, 1)

        layOne = QtWidgets.QVBoxLayout(self.m_w11)
        layOne.addWidget(self.TextOne)
        ...
        # setting stats bar to the winow
        ...
        open_file_action = QAction(
            QIcon('images/folder.jpg'), "Open file", self)
        ...
        save_file_action = QAction(QIcon('images/save.png'), "Save", self)
        ...
        saveas_file_action = QAction(
            QIcon('images/saveas.png'), "Save As", self)
        ...
        # undo action
        undo_action = QAction(QIcon('images/undo.webp'), "Undo", self)
        # adding status tip
        undo_action.setStatusTip("Undo last change")

        # when triggered undo the TextOne
        undo_action.triggered.connect(self.TextOne.undo)
        ...
        # redo action
        redo_action = QAction(QIcon('images/redo.webp'), "Redo", self)
        redo_action.setStatusTip("Redo last change")

        # when triggered redo the TextOne
        redo_action.triggered.connect(self.TextOne.redo)
        ...
        # when triggered cut the TextOne text
        cut_action.triggered.connect(self.TextOne.cut)
        ...
        # when triggered copy the TextOne text
        copy_action.triggered.connect(self.TextOne.copy)
        ...
        paste_action.triggered.connect(self.TextOne.paste)
        ...
        select_action = QAction(
            QIcon('images/selectall.png'), "Select all", self)
        select_action.setStatusTip("Select all text")

        # when this triggered select the whole text
        select_action.triggered.connect(self.TextOne.selectAll)
        ...
        if path:
            ...
            try:
                with open(path, 'r') as f:
                    # read the file
                    text = f.read()
                    self.TextOne.setPlainText(text)
            ...
            # else
                ...
                self.path = path
                # update the text
                # self.TextOne.document().setPlainText(text)
    ...
    def file_print(self):
        ...
        if dlg.exec_():
            ...
            # print the text
            self.TextOne.print_(dlg.printer())
    ...
    def update_title(self):
        ...
        self.setWindowTitle("%s - FastNote" % (os.path.basename(self.path)
    ...
    def edit_toggle_wrap(self):
        ...
        self.TextOne.setLineWrapMode(
            1 if self.TextOne.lineWrapMode() == 0 else 0)
